...
**Название** — content-strateg (контент-завод): фреймворк субагентов Claude Code для контент-конвейера.

**Проблема** — производство контента у эксперта/редактора хаотично: нет единого процесса от идеи
до публикации, факты не проверяются системно, легко нарушить закон о рекламе или авторское право,
а у клиентских проектов свои жёсткие редполитики, которые держатся «в голове». Нужен управляемый
конвейер с встроенными проверками качества и человеческими чекпоинтами.

**Пользователь** — журналист / редактор / эксперт, ведущий личный бренд и клиентские проекты
(медиаправо, контент-маркетинг, образование), работающий в Claude Code.

**Основная функция** — провести материал по стадиям `стратегия → ресёрч → производство →
fact-check → экспертное ревью → редактура → дистрибуция` силами 6 субагентов под управлением
дирижёра `/content-pipeline`, с паузой на «да» владельца после каждой стадии.

**Формат результата** — готовый к публикации пакет в папке проекта: проверенная фактура, материалы
под площадки ядра, план дистрибуции, статусы fact-check и ревью в `pipeline.md`. Система **ничего
не публикует сама**.

**Минимальный функционал** — 6 агентов + дирижёр + 14 шаблонов + методология в `CLAUDE.md` (классы
контента, иерархия источников, запрет фабрикации и публикации, биндинг клиентской редполитики,
человеческая стадия экспертного ревью с правом вето).

**Технологии** — Markdown + YAML-фронтматтер, Claude Code sub-agents и slash-команды, модели
Opus / Sonnet по нагрузке; опциональные read-only MCP; Git + GitHub CLI. Лицензия CC BY 4.0.

> Полная версия брифа — [docs/about.md](docs/about.md).
